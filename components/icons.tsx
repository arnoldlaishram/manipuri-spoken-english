 ...
 *
 * The button used to carry a plain dot. She cannot read the English label on
 * it, so the shape has to do the work: a mic means speak. When it is listening
 * the sound waves appear beside it.
 */
export function MicIcon({ live = false }: { live?: boolean }) {
  return (
    <span className={`micglyph${live ? " live" : ""}`} aria-hidden="true">
      ...
      {/* only while it is genuinely listening */}
